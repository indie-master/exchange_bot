...
После этого `CBR-rates` можно запускать из любого места. Если вы уже находитесь в каталоге репозитория, достаточно выполнить `CBR-rates` (или даже `python CBR-rates`) — скрипт сам найдёт корень проекта. Для единичного вызова вне репозитория можно передать путь напрямую, не меняя переменные:
